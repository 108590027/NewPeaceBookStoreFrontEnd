...
export function toInputDate(timestamp: number): string {
    ...
    return `${date.getFullYear()}-${`${(date.getMonth()+1)}`.padStart(2, '0')}-${`${(date.getDate())}`.padStart(2, '0')}T${`${(date.getHours())}`.padStart(2, '0')}:${`${(date.getMinutes())}`.padStart(2, '0')}:${`${(date.getSeconds())}`.padStart(2, '0')}.000Z`.slice(0, 19);
} 
